					...
					for _, ptrUse := range getUses(inttoptr) {
						if !ptrUse.IsABitCastInst().IsNil() {
							for _, bitcastUse := range getUses(ptrUse) {
								...
								case "runtime.makeGoroutine":
									for _, bitcast := range getUses(bitcastUse) {
										if bitcast.IsABitCastInst().IsNil() {
											panic("expected a bitcast")
										}
										for _, use := range getUses(bitcast) {
											c.addFuncLoweringSwitch(funcID, use, c.createGoCall, functions)
